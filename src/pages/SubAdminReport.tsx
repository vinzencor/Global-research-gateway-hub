...
export default function SubAdminReport() {
  ...
  const score = approved * SCORE_APPROVED + changesReq * SCORE_CHANGES + rejected * SCORE_REJECTED;
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { label: "Approval Rate", value: `${approvalRate}%`, icon: <CheckCircle className="h-5 w-5 text-success" />, color: "text-success" },
            {
              label: "This Month",
              value: thisMonth,
              icon: <TrendingUp className={`h-5 w-5 ${trend >= 0 ? "text-success" : "text-destructive"}`} />,
              color: trend >= 0 ? "text-success" : "text-destructive",
              sub: trend !== 0 ? `${trend > 0 ? "+" : ""}${trend} vs last month` : "Same as last month"
            },
          ].map((s) => (
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 rounded-xl border bg-card p-6 card-shadow">
            ...
            {monthlyData.every(m => m.count === 0) && (
              <p className="text-sm text-muted-foreground text-center mt-4">No activity in the last 6 months</p>
            )}
          </div>
          ...
          <div className="rounded-xl border bg-card p-6 card-shadow">
            ...
            <div className="space-y-4">
              {[
                { label: "Approved", count: approved, total, color: "bg-success", textColor: "text-success", pts: SCORE_APPROVED },
                { label: "Changes Requested", count: changesReq, total, color: "bg-orange-500", textColor: "text-orange-600", pts: SCORE_CHANGES },
                { label: "Rejected", count: rejected, total, color: "bg-destructive", textColor: "text-destructive", pts: SCORE_REJECTED },
              ].map(item => {
                    ...
                    <tr key={log.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                      ...
                      <td className="p-4 hidden sm:table-cell">
                        <Badge variant="outline" className={`text-xs ${
                          ...
                          log.action === "changes_requested" ? "bg-orange-500/10 text-orange-600 border-orange-200" :
